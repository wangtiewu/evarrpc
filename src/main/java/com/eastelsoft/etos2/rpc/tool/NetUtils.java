...
public class NetUtils {
	...
	public static boolean isLocalIp(String address) {
		...
		try {
			mArLocalIP = InetAddress.getAllByName(InetAddress.getLocalHost()
					.getHostName());
			for(InetAddress l : mArLocalIP) {
				if (l.getHostAddress().equalsIgnoreCase(address)) {
					result = true;
					break;
				}
			}
	...
	private static InetAddress getLocalAddress0() {
		...
		logger
				.error("Could not get local host ip address, will use 127.0.0.1 instead.");
		return localAddress;
	}

	private static final Cache hostNameCache = new MapCache();
	{
		hostNameCache.init();
	}
	...
	public static String getHostName(String address) {
		try {
			...
			String hostname = (String) hostNameCache.getFromCache(address);
			...
			if (inetAddress != null) {
				...
				hostNameCache.putInCache(address, hostname);
				return hostname;
			}
		} catch (Throwable e) {
			// ignore
	...
	public static void main(String[] args) {
		String ip = "10.10.8.22";
		int port = 6003;
		
		System.out.println(NetUtils.getAvailablePort(ip, port));
	}
